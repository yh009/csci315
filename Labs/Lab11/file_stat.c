...
int main(int argc, char *argv[]) {
  ...
  if (fstat(fd, &file_info) == 0) {
    ...
    printf(" file protection mode (u:g:o) = %o:%o:%o\n",
	   umode, gmode, omode);
    char *buf = malloc(sysconf(_SC_GETPW_R_SIZE_MAX));
    getpwuid_r(file_info.st_uid,&pwd,buf,sysconf(_SC_GETPW_R_SIZE_MAX),&result);
    printf(" owner user name = %s\n",pwd.pw_name); // TO-DO: man getpwuid 
    getgrgid_r(file_info.st_uid,&grp,buf,sysconf(_SC_GETPW_R_SIZE_MAX),&result_g);
