...
void *SumExp(void *args_ptr) {
    ...
    time_t the_end_p = (time_t)&end_p;
    printf("Thread %d processing time: %li microseconds\n",myargs_ptr->tid,(end_p.tv_sec-start_p.tv_sec)*1000000+(end_p.tv_usec-start_p.tv_usec));
    printf("Thread %d end time = %s",myargs_ptr->tid,ctime(the_end_p));
